...
export const TutoringModule = () => {
  ...
  const handleEnroll = (courseId) => {
    ...
    registerTutoring(courseId, currentStudent.id, customFeeInput || 50);
    setSuccessToast(isAr ? `تم تسجيل التلميذ (${currentStudent.name}) وقسطه ($${customFeeInput || 50} USD) بنجاح! 🟢` : 'Registration saved!');
  ...
  const handleRecordPaymentSubmit = (e) => {
    ...
    setPayAmountInput('');
    setSuccessToast(isAr ? 'تم تسجيل الدفعة المالية للتلميذ في حسابات التقوية بنجاح! 💰' : 'Payment recorded!');
    setTimeout(() => setSuccessToast(''), 3500);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in text-[#0F172A]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              {isAr ? 'تسجيل التلاميذ وإدارتهم في دورات التقوية وتحديد القسط المالي المخصص لكل تلميذ وحفظه في حسابات خاصة.' : t('tutoringSubtitle')}
            </p>
          </div>
        </div>
      ...
      {successToast && (
        <div className="bg-emerald-50 border border-emerald-300 text-emerald-800 p-4 rounded-2xl flex items-center gap-3 shadow-lg animate-fade-in text-xs font-bold">
      ...
      {activeTab === 'courses' && (
        <div className="space-y-6">
          ...
          <div className="bg-white border border-[#E2E8F0] p-4 rounded-3xl shadow-sm text-[#0F172A] flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-xs font-bold text-[#0284C7]">
              <UserPlus className="w-4 h-4" />
              <span>{isAr ? 'تسجيل تلميذ وسداد القسط المخصص:' : 'Register Student & Assign Fee:'}</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              <select
                ...
                className="bg-[#F8FAFC] border border-[#E2E8F0] text-xs font-bold text-[#0F172A] rounded-2xl px-3 py-2 focus:outline-none focus:border-[#0284C7] cursor-pointer flex-1 md:flex-none min-w-[200px]"
              >
                {safeStudents.map((s) => (
              ...
              <div className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E2E8F0] px-3 py-2 rounded-2xl">
                <span className="text-xs font-black text-slate-500">$</span>
                <input
                  type="number"
                  placeholder={isAr ? 'القسط المخصص' : 'Fee $'}
                  value={customFeeInput}
                  onChange={(e) => setCustomFeeInput(e.target.value)}
                  className="w-24 text-xs font-mono font-bold bg-transparent focus:outline-none text-right text-[#0284C7]"
                />
                ...
                >
                  ...
                  <div className="pt-3 border-t border-slate-100">
                    {isEnrolled ? (
                      ...
                      >
                        <UserPlus className="w-4 h-4" />
                        <span>{isAr ? `تسجيل (${currentStudent?.name || 'التلميذ'}) بالقسط ($${customFeeInput || 50})` : "Register Student Now"}</span>
                      </button>
                    )}
                  </div>
                </div>
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            {currentRole === 'admin' && (
                              <button
                                ...
                                title="تسديد دفعة مالية لحساب التقوية"
                              >
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[99999] flex items-center justify-center p-4 overflow-y-auto">
          <form onSubmit={handleRecordPaymentSubmit}
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button type="submit" className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow cursor-pointer">تسجيل الدفعة 💰</button>
            </div>
          </form>
        </div>,
        document.body
